...
Mục tiêu 1: Đảm bảo mỗi hệ có số lượt đánh tối thiểu theo từng level:
- Kéo: level 1 đạt trung bình 7 lượt đánh, tăng dần đều đến level 40 trung bình 14 lượt đánh
- Búa: level 1 8.5 lượt, level 40 đạt 18 lượt
- Bao: level 1 7.5 lượt, level 40 đạt 16 lượt

Mục tiêu 2: Đảm bảo các kèo khắc chế luôn thắng (dù đánh trước hay đánh sau, tỉ lệ thắng trung bình tốt nhất nên đạt 100%, thấp nhất 95%) với lượng HP còn lại của bên khắc chế dao động 10-50% trên toàn bộ giai đoạn (với HP còn lại, chỉ quan tâm khi level >= 10, dưới level 10 thì không cần quan tâm, vì đây là giai đoạn người chơi chưa được phép đánh PvP).
...
Bạn không được phép thay đổi vòng tròn khắc chế giữa các hệ (Búa > Kéo > Bao > Búa) và cơ chế cùng hệ "We are family" (Kéo vs Kéo, Búa vs Búa, Bao vs Bao). Các cơ chế này phải được giữ nguyên, nhưng bạn có thể chỉnh sửa chi tiết bên trong để đạt được mục tiêu. Lưu ý rằng bạn chỉ được phép thay đổi tối đa 2 chỉ số trong 4 chỉ số (hp, atk, def, luck) của từng hệ để đạt được mục tiêu. Nếu cần thay đổi nhiều hơn 2 chỉ số thì phải cho thông báo cho tôi biết trước khi thực hiện.

Yêu cầu: viết lại 1 phiên bản hoàn toàn mới của combat_simulator (version 10, viết mới hoàn toàn) sau đó cân bằng đến khi đạt cả 2 mục tiêu ở trên, kiểm tra trên tập ít nhất 1000 trận đấu hoặc hơn (bao gồm bên khắc đi trước và đi sau), ở các mốc level 1, 5, 10, 15, 20, 25, 30, 35, 40. Không cần UI (tức không cần file html và css, chỉ cần chạy console), chỉ cần kết quả report khi chạy, không cần tạo file report .md. Kết quả report cần bao gồm:
...
Được phép tham khảo thông tin từ v9 (chỉ tham khảo, không sao chép).
